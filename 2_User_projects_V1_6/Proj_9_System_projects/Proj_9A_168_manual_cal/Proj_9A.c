...
setup_HW_E;													//"setup_HW_E;"initially saves default OSCCAL in 0x1F9 and
OSCCAL_DV = eeprom_read_byte((uint8_t*)0x1F9);				//then checks for "user-cal" and copies it to OSCCAL if present
...
eeprom_write_byte((uint8_t*)0x1F7, 0xFF);					//Delete user cal value
eeprom_write_byte((uint8_t*)0x1F8, 0xFF);
SW_reset;}
