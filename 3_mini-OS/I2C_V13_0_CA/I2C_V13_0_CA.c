...
Following POR check cal factor and resets it if necassary
Adds mode U: quick cal restore

***********************/


# include "I2C_V13_0_CA_Header_file.h"
...
char *SW_Version = "OS: I2C_V13_0_CA\r\n";
char *SW_info = "SW information: Operating system I2C_V13_0_CA\
  Projects V1_4 and Bootloader V4_21_CA. (zero_SUT & auto clock recovery) \
  External programmer V2_29A\r\n";
...
/*************Check OSC Calibration and reset if necessary*****************************/
...
if((MCUSR & (1 << PORF))){									//Only run clock check following a POR
	...
	mode = 'U';
		initialise_timers_for_cal_error();
		start_timers_for_cal_error();
		for(int m = 0x10; m <= 0xF0; m++){OSCCAL = m ;cal_error = emergency_restore_operation();
		if (cal_error >= 0) error_mag = cal_error; else error_mag = cal_error * (-1);
		if ((error_mag < 750) && (m > 0x14))break;}	//{OSCCAL_WV = m;break;}	}
		mode = 'O';
		OSCCAL_test = OSCCAL;
	initialise_timers_for_cal_error();  				
	start_timers_for_cal_error();
	cal_error = calibrate_quick_cal(OSCCAL_test);				//Adjusts OSCCAL
	if(cal_error > 500)cal_error = calibrate_quick_cal(OSCCAL + 4);				//Adjusts OSCCAL
...
case 'W':																	//W and X are both called by setup_HW
case 'w':
