...
//Manual calibration of ATMEGA 168 performed on propgramming PCB
...
#include "Cal_V2_5_header_file.h"
#define Version "SW V2_5"
int main (void){
...
sendString("\r\nPerforming  manual calibration on ATMEGA168  ");
...
sendString("DC value  "); sendLongNum(16, (eeprom_read_byte((uint8_t*)0x1F9))); }
...
if (waitforkeypress() == 'y')
	...
	((error = compute_single_error(New_UC_value, OSCCAL_WV)) >= 1500)){
	newline(); sendLongNum(10, error); sendString("  Error too great, try again");
	newline();SW_reset;}	
	
	eeprom_write_byte((uint8_t*)0x1F7, New_UC_value); 
	eeprom_write_byte((uint8_t*)0x1F8, New_UC_value); 
	if(New_UC_value == 0xFF) OSCCAL = OSCCAL_WV; else OSCCAL = New_UC_value;}
else {SW_reset;}}
		
sendString("\r\nValues saved to EEPROM\t");				//Echo values back from the EEPROM
sendLongNum(16,eeprom_read_byte((uint8_t*)0x1F7));
sendString ("    ");
sendLongNum(16,eeprom_read_byte((uint8_t*)0x1F8));
sendString("\r\nAK to repeat\r\n");
