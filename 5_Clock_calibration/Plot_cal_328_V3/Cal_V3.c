...
#include "Cal_V3_header_file.h"
#define Version "SW V3"
int main (void){
char cal_factor=0;
...
sendString("\r\nGenerate plot of ATMEGA328 calibration factors  ");
...
Timer_T0_sub(T0_delay_5ms);									//Allows printing to finish before OSCCAL changes
		...
		/*for(int m = 0x0F; m <= 0xF0; m++)					//Print table of values
{sendLongNum(10,m); sendChar('\t');
sendLongNum(10,buffer[m]); sendChar('\t');
sendLongNum(10,buffer[m]*100/62500);
sendChar('%');newline();}*/
